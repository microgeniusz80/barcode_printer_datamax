...
def update_test(test_id, test_name, test_date, result):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""
        UPDATE tests
        SET test_name=?, test_date=?, result=?
        WHERE id=?
...
class LISApp(tk.Tk):
    def __init__(self):
        ...
        frm_pat.place(x=10, y=10, width=440, height=220)
        ...
        self.e_barcode = ttk.Entry(frm_pat, width=34)
        self.e_ward    = ttk.Entry(frm_pat, width=34)
        self.e_name    = ttk.Entry(frm_pat, width=34)
        self.e_ic      = ttk.Entry(frm_pat, width=34)
        ...
        frm_test.place(x=10, y=240, width=440, height=310)
        ...
        # Combobox for test names
        self.combo_test_name = ttk.Combobox(frm_test, values=TEST_CHOICES, state="readonly", width=31)
        self.combo_test_name.grid(row=0, column=1, padx=6, pady=6)
        self.combo_test_name.set(TEST_CHOICES[0])

        self.e_test_date = ttk.Entry(frm_test, width=34)
        ...
        self.e_result = tk.Text(frm_test, width=32, height=6)
        ...
        frm_list.place(x=460, y=10, width=450, height=540)
        self.tests_tree = ttk.Treeview(frm_list, columns=("id","name","date","result"), show="headings", height=22)
        ...
        self.tests_tree.column("name", width=140)
        self.tests_tree.column("date", width=90, anchor="center")
    ...
    def on_tree_select(self, event):
        ...
        test_id, tname, tdate, result = item["values"]
        self.selected_test_id = test_id
        # Fill form with selected test
    ...
    def update_test_btn(self):
        ...
        if not tname or not tdate:
            messagebox.showerror("Error", "Test name and date are required.")
            return
        update_test(self.selected_test_id, tname, tdate, result)
        self.refresh_tests()
