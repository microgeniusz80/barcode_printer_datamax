...
# =========================
# GUI (Tkinter) – previous layout, with bottom bar
# =========================
class LISApp(tk.Tk):
    def __init__(self):
        super().__init__()
        ...
        self.tests_tree.pack(side="top", fill="both", expand=True, padx=6, pady=6)
        self.tests_tree.bind("<<TreeviewSelect>>", self.on_tree_select)

        # --- Bottom bar so controls are ALWAYS visible below the list ---
        bottom_bar = ttk.Frame(frm_list)
        bottom_bar.pack(side="bottom", fill="x", padx=6, pady=(0,6))

        btn_refresh = ttk.Button(bottom_bar, text="Refresh List", command=self.refresh_tests)
        btn_refresh.pack(side="top", fill="x", pady=(0,4))

        # Current price label
        self.current_price_label = ttk.Label(bottom_bar, text="Current price: RM 0.00", anchor="e")
        self.current_price_label.pack(side="top", fill="x", pady=(0,4))

        # Total payment label (large/bold)
        self.total_label = ttk.Label(bottom_bar, text="Total: RM 0.00", anchor="e")
        try:
            big = ("Segoe UI", 12, "bold")
        except:
            big = ("TkDefaultFont", 12, "bold")
        self.total_label.configure(font=big)
        self.total_label.pack(side="top", fill="x")
